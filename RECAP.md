...
**Date:** 7 May 2026 · **Last update:** 21 May 2026 (Railway deploy live; Supabase project re-pointed; history sanitised)
...
**Code:** 🟢 H-1 + H-2 + H-3 + H-3.1 + H-3b + H-5 + H-6 + H-7 + **H-8** closed (H-8a rail-side + H-8b Console RN library foundation; UI layer deferred — see §6.19).
...
- **DB:** migrations 0001–**0009** apply cleanly. Currently pointed at Supabase project `jvkhoveeayixbjnhmqxa` (eu-west-1, session pooler port 5432) — **provisioned 19 May 2026, migrations not yet re-run against this project** (the earlier project `oklovgfjrnsfxrlcfapg` had them applied; we switched projects 20 May 2026). 0009 added `business_op_id` to `audit_log` + `actions` (§A.11 cross-rail audit join). RLS enforced on `briefings` (FORCE) and `audit_log` (operator-only SELECT).
...
- **Tests:** rail **173/173 green** (90 always-on + 83 real-Postgres against `helpan_ai_test` on `localhost:5432`); Console foundation **43/43**. Typecheck + lint clean across both.
...
- **Next:** (i) re-run `npm run db:migrate` against the new Supabase project so 0001–0009 apply to `jvkhoveeayixbjnhmqxa`; (ii) seed first `app_credentials` row (otherwise every HMAC endpoint 401s `AUTH_UNKNOWN_APP`); (iii) finish H-8b component layer. H-4 (action dispatch) hard-blocked on **KP KP-8 + Todoku TD-1**; H-9–H-12 on KP/Todoku; H-13 on H-8. H-8b is the only buildable sprint work in flight.
