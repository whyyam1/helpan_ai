...
**Code:** 🟢 H-1 + H-2 + H-3 + H-3.1 + H-3b + H-5 + H-6 + H-7 closed (H-3.1 + H-3b closed 18 May 2026).
...
- **Auth:** HMAC (rail-wide default) + Identiti customer-JWT on `/v1/briefings/*` only. Admin surfaces require `helpan:admin`; authority endpoints require per-endpoint scopes — all enforced by rail-local `requireScope(request, scope)`. `/v1/authorities/*` is all-HMAC in v1.0 (H-3 decision 1a; customer-JWT Console list/revoke deferred to H-8).
- **DB:** migrations 0001–**0009** apply cleanly — applied to the Supabase project (`oklovgfjrnsfxrlcfapg`, eu-west-1, Postgres 17.6) and the local test DB. 0009 added `business_op_id` to `audit_log` + `actions` (§A.11 cross-rail audit join). RLS enforced on `briefings` (FORCE) and `audit_log` (operator-only SELECT).
- **Audit chain:** writer (`src/lib/auditWriter.ts`) extended at H-3.1 with the §A.11 indexed fields — `agentId`, `delegatedAuthorityJti`, `targetRail`, `targetOperation`, `businessOpId`. `authority.issue` / `authority.revoke` now populate `agent_id` + `delegated_authority_jti` columns (not just `detail` JSON). `entry_hash` composition unchanged — new columns follow the existing partial-hash design (see §6.13).
...
- **Kafka:** producer surface in `src/lib/kafka/{topics,producer}.ts` — `kafkajs` prod / in-memory test stub. Cascade worker adds the first *consumer* (`identiti.account.events`, group `helpan.identiti.account.events.consumer`). Producer/consumer optional — missing brokers → publishes skipped, DB + webhook paths still work; the cascade worker exits 1 without brokers (nothing to consume).
- **Tests:** **163/163 green** — 80 always-on + 83 real-Postgres against `helpan_ai_test` on `localhost:5432`. Typecheck clean, lint clean.
- **Deploy:** `@kmv/platform-shared` vendored to `vendor/platform-shared/`; GitHub `whyyam1/helpan_ai`; Railway provisioning in progress.
- **Next:** H-4 (action dispatch) is hard-blocked on **KP KP-8 + Todoku TD-1**. H-8 (Helpan Console RN library) is unblocked but is a separate React Native codebase needing its own plan. Every other locally-buildable item is done.
...
| Real-Postgres **staging** smoke (Supabase) | 18 May 2026 | `.env` wired to the Supabase project `oklovgfjrnsfxrlcfapg` (eu-west-1, Postgres 17.6); migrations 0001–0009 applied; 14 rail tables + audit genesis + 11 seeded scopes verified. The dev-only `localhost` smoke (H-2) is now backed by a real staging DB. |
